...
class UserSpace(object):
    ...
    def user_cost(self,author,users):
        print "In User Class"
        k=0.0
        for i in range(self.input.ndim):
          Sample =  random.randint(1, users.ndim-1)
          # k.append( T.log(T.dot(self.input[i],y[i])) - T.log(T.dot(self.input[i],y[Sample])))
          k = k +T.log(T.nnet.sigmoid(T.dot(self.input[i],author[i])))+ T.log(T.nnet.sigmoid(-T.dot(self.input[i],users[Sample])))

        return -(k/self.input.ndim)

    def user_graph_cost(self,author,user_graph,users):
        print "In User Graph Class"
        Negative = 5
        k=0.0
        for i in range(self.input.ndim):

          for followers in user_graph[i]:
               k = k +T.log(T.nnet.sigmoid(T.dot(author[i],users[followers])))

          for  neg in range(Negative):
               Sample =  random.randint(1,users.ndim-1)
               if Sample not in  user_graph[i]:
                    k = k + T.log(T.nnet.sigmoid(-T.dot(author[i],users[Sample])))
